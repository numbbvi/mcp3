  ...
  scanCode: async (req, res) => {
    try {
          ...
          if (targetServerName && req.user && req.user.id) {
            try {
              ...
              if (apiKeys && apiKeys.length > 0) {
                ...
                // .env 파일 내용 생성 (Settings에서 등록한 API 키)
              ...
              if (code === 0) {
                ...
                try {
                  ...
                  if (reportFile) {
                    ...
                    console.log(`[TOOL-VET] 서버 이름 추출: mcpServerName=${mcpServerName}, serverName=${serverName}, mcp_server_name=${mcp_server_name}, github_url=${github_url}`);
                    ...
                    if (reportData.tools && Array.isArray(reportData.tools)) {
                      try {
                        ...
                        let finalServerName = mcp_server_name;
                        ...
                        if (!finalServerName && scanId) {
                          ...
                          if (scanRecord && scanRecord.mcp_server_name) {
                            finalServerName = scanRecord.mcp_server_name;
                            console.log(`[TOOL-VET] tool_validation_reports에서 서버 이름 찾음: ${finalServerName}`);
                          }
                        }
                        
                        if (!finalServerName) {
                          console.error('[TOOL-VET] 서버 이름을 찾을 수 없어 tool 정보 저장 스킵');
                          console.error(`[TOOL-VET] 조회 시도 정보: mcp_server_name=${mcp_server_name}, scanPath=${scanPath}, scanId=${scanId}`);
                          return;
                        }
                        
                        console.log(`[TOOL-VET] 최종 서버 이름 (사용자 입력): ${finalServerName}`);
                        
                        console.log(`[TOOL-VET] 최종 서버 이름: ${finalServerName}`);
                        ...
                        if (mcpServerId) {
                          console.log(`[TOOL-VET] MCP 서버 ID 찾음: ${finalServerName} -> ${mcpServerId}`);
                        } else {
                          console.log(`[TOOL-VET] MCP 서버가 아직 승인되지 않음: ${finalServerName} (server_name으로 저장)`);
                        }
                        
                        // 기존 tools 삭제 (같은 서버 이름의 이전 tool 정보)
                        const deleteToolsStmt = db.prepare('DELETE FROM mcp_tools WHERE server_name = ?');
                        deleteToolsStmt.run(finalServerName);
                        console.log(`[TOOL-VET] 기존 tool 정보 삭제 완료: server_name=${finalServerName}`);
                        ...
                        for (const tool of reportData.tools) {
                          ...
                          if (toolName) {
                            try {
                              insertToolStmt.run(mcpServerId, finalServerName, toolName, toolDescription, inputSchema);
                              savedTools++;
                              console.log(`[TOOL-VET] Tool 저장 완료: ${toolName} (server_name: ${finalServerName})`);
                            } catch (toolError) {
                        ...
                        console.log(`[TOOL-VET] ${savedTools}개 tool 정보 저장 완료: server_name=${finalServerName}${mcpServerId ? `, mcp_server_id=${mcpServerId}` : ''}`);
      ...
      try {
        ...
        if (finalProgress) {
          // TOOL-VET만 실행 (Bomtori와 Scanner는 주석처리됨)
          if (finalProgress.toolVetError) {
            finalProgress.status = 'failed';
            finalProgress.error = finalProgress.toolVetError;
          } else {
